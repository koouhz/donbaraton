-- =====================================================
-- FIX: Stock No Vendible para Devoluciones
-- EJECUTAR TODO ESTE SCRIPT EN SUPABASE SQL EDITOR
-- 
-- Este script soluciona el problema donde los productos
-- devueltos por "Dañado" o "Vencido" no aparecían en
-- la sección de Stock No Vendible.
-- =====================================================

-- =====================================================
-- PASO 1: Crear tabla stock_no_vendible (si no existe)
-- =====================================================
...
-- =====================================================
-- PASO 3: Función CORREGIDA de devolución parcial
-- ESTA ES LA FUNCIÓN QUE SÍ INSERTA EN stock_no_vendible
-- =====================================================
...
BEGIN
    ...
    LOOP
        ...
        ELSIF UPPER(p_motivo) LIKE '%DAÑA%' OR UPPER(p_motivo) LIKE '%DAÑO%' OR UPPER(p_motivo) = 'DAÑADO' THEN
        ...
        ELSIF UPPER(p_motivo) LIKE '%VENCIDO%' OR UPPER(p_motivo) = 'VENCIDO' THEN
...
-- =====================================================
-- VERIFICACIÓN: Comprobar que todo está configurado
-- =====================================================
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'stock_no_vendible') THEN
        RAISE NOTICE '✅ Tabla stock_no_vendible creada correctamente';
    ELSE
        RAISE NOTICE '❌ ERROR: La tabla stock_no_vendible no se creó';
    END IF;
END $$;

-- Verificar función fn_devolucion_venta_parcial
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'fn_devolucion_venta_parcial') THEN
        RAISE NOTICE '✅ Función fn_devolucion_venta_parcial actualizada correctamente';
    ELSE
        RAISE NOTICE '❌ ERROR: La función fn_devolucion_venta_parcial no existe';
    END IF;
END $$;

-- Verificar función fn_leer_stock_no_vendible
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'fn_leer_stock_no_vendible') THEN
        RAISE NOTICE '✅ Función fn_leer_stock_no_vendible creada correctamente';
    ELSE
        RAISE NOTICE '❌ ERROR: La función fn_leer_stock_no_vendible no existe';
    END IF;
END $$;
...
-- NOTAS IMPORTANTES:
-- 
-- 1. Este script actualiza la función fn_devolucion_venta_parcial para
--    insertar automáticamente en stock_no_vendible cuando el motivo
--    de devolución es "DAÑADO" o "VENCIDO"
--
-- 2. Los productos devueltos por "Error de compra" vuelven al stock
--    vendible normal (comportamiento original)
--
-- 3. Los productos dañados/vencidos se almacenan en stock_no_vendible
--    con estado "PENDIENTE" hasta que se gestionen manualmente
--
-- 4. Después de ejecutar este script, las nuevas devoluciones con
--    motivo dañado/vencido aparecerán automáticamente en la sección
--    "Stock No Vendible" del sidebar
-- =====================================================
